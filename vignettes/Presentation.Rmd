...
a. Podemos descargar sobre un usuario determinado con la función *get_timeline_data*
```{r}
...
```
c. E incluso podemos hacerlo para ver el timeline de más de un usuario:
```{r}
timeline_af_mm <- get_timeline_data(screen.name = c("alferdez", "mauriciomacri"))
head(timeline_af_mm)
```
d. Y graficar en qué periodos de tiempos más twittearon ambos:
```{r}
# timeline_mev_mm %>%
...
4. E incluso podemos ver cuántos amigos tiene cada uno:
```{r}
library(sknifedatar)
...
#             geom_area(aes(fill=screen_name), alpha=0.5) +
#             geom_line(aes(color=screen_name)) +
#             ylab("Followers obtenidos por @alferdez y @mauriciomacri") +
#             theme_ipsum()
```
